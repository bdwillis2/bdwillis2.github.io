---
title: 'Predicting Heart Failure through clincal data'
author: 'Berkeley D. Willis'
output: html_document
---

This similar in nature to a previous project where I had looked and did analysis on heart failure data. However, this time I am going to look further, and at new features to see if it is possible to accurately predict death from heart disease. To do this I will be building multiple models, and to avoid issues in the slight randomization of the splits they will be run multiple times and with the results averaged.

First I will need to get the libraries required to not only build the models but to manipulate the data.
...
With the libraries in place, it is now time to pull the data from the filesystem, and start doing some basic operations on it.

For many of the models, for the simplicity that is, many of the numbers to boolean values. This will make it a bit easier to look at in visualization or in tabular form, and when aggregated it will seem a bit more straight forward.

Other data cleaning operations here will be things like converting to integers or treating the sex variable as a factor.
...
With the data in a better place we can start to look at some various aspects of the data.

Quick easy thing to chekcout is a summary
```{r data_summary}
hf_records_dt %>% summary
```

This gives a good idea of what is available and the general state of the data. Nothing is necessarily going to be conclusive here, but we can see certain aspects of the data. For example there is a particular gender that is more represented here, and many don't really suffer from high blood pressure.

With that said it is better to be able to see some basic visualizations showing the correlative heat map, and some visualizations of scatter plots between a set of variables.
...
Now that there is a better understanding of the relationships, and of course some assumptions from some interpretations of what each measure represents, models can be created.

The basic idea here is going to be trying to take a bunch of the features, and slowly pruning them back and seeing what the effect is overall.
...
Now with the formulas we can start to build some of the functions that will help run the test many times to get an average and go ahead with the test.

```{r regression_model_functions}
...
Now  Iwant to see what the difference would be if decisions trees were utilized.
...
Now for measure the accuracy of SVMs
...
Finally, just a curiosity of mine. I am wondering with a combination of models, how would an ensemble being built from these models would performs. Woudl this increase their performance like just taking the average between the two raw outputs of the models, or does this lower the performance?
...
With all of the information that we have gathered, it seems that simplicity is of course best. With this  Ihave removed all but the features that seem to perform the best.
...
It is interesting how much change time injects here, I am curious how this feature is decided. Is there something a doctor is looking at that we don't have access to at the moment in order to know that they should see this patient more often/sooner? Is what the doctor looking at available in raw data form, and then I would love to test the models using that data. The models built here are not really performing to the best possible, but they are hopefully something that could give an idea of those that would need to be monitored closer.
