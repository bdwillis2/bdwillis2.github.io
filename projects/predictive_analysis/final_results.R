...
ggplot(hf_records_dt, aes(x=ejection_fraction,y=serum_creatinine, color=DEATH_EVENT))+
    geom_point()

ggplot(hf_records_dt[ hf_records_dt$DEATH_EVENT ], aes(x=ejection_fraction,y=serum_creatinine, color=DEATH_EVENT))+
    geom_point()
...
# Testing ensemble models, but very simple ones
buildEnsemble = function(FUNCTION=mean, data_df, iters=10000, split_p=0.8, ...){
  # First going to build the logistic models based on provided formulas
  accs = sapply(1:iters, function(x, p_df, split, formulas_l){
    # First make sure param is used to avoid for loops
    y = x
    # Split the data
    index = sample(1:nrow(p_df), size=round(nrow(p_df)*split))
    # Now go through each one and build a model
    models_l = lapply(formulas_l, function(formula, model_data){
      glm(formula, model_data, family=binomial(link="logit"))
    }, model_data=p_df[index,])
    # For each model, predict them
    predictions_l = lapply(models_l, function(model, predict_data){
      predict(model, predict_data, type="response")
    }, predict_data=p_df[!index,])
    # Now attempting to aggregate for each entry, kinda messy way of doing it, but fairly simple when broken down
    # Going through each list, getting the transformation of each prediction vector, and treating is a dataframe, then
    # combined into a dataframe via a plyr rbind.fill function
    predictions_df = rbind.fill(
      lapply( predictions_l,function(prediction_c){
        as.data.frame(t(prediction_c))
      })
    )
    # Then you can just take each column and run the FUNCTION, likely the mean function
    ensembled_preds = apply(predictions_df, 2, FUNCTION)
    # After that, now we can normalize/round the prediction values
    final_preds = round(ensembled_preds)
    tmp_table = table(final_preds, as.integer(unlist(p_df[!index, "DEATH_EVENT"])))
    # Missing rows?
    fill_row = setdiff(colnames(tmp_table), rownames(tmp_table))
    if(length(fill_row)!=0){
      tmp_table = rbind(tmp_table, tmp_row=c(0,0))
      rownames(tmp_table)[ rownames(tmp_table)=="tmp_row" ] = fill_row
      tmp_table = as.table(tmp_table)
    }
    # Then create confusion matrices
    conf_mat = confusionMatrix(tmp_table)
    conf_mat$overall[1]
  }, p_df=data_df, split=split_p, formulas_l=list(...))
  print(paste("Average accuracy over", iters, "iterations:",mean(accs)))
}

buildEnsemble(mean, hf_records_dt, 10000, 0.8, formula1)
buildEnsemble(mean, hf_records_dt, 10000, 0.8, DEATH_EVENT ~ age, DEATH_EVENT ~ creatinine_phosphokinase + serum_creatinine, DEATH_EVENT ~ ejection_fraction, DEATH_EVENT ~ serum_sodium)
buildEnsemble(mean, hf_records_dt, 10000, 0.8, DEATH_EVENT ~ age + high_blood_pressure + sex + smoking, DEATH_EVENT ~ anaemia + creatinine_phosphokinase + ejection_fraction + serum_creatinine  + serum_sodium)
buildEnsemble(mean, hf_records_dt, 10000, 0.8, DEATH_EVENT ~ age + high_blood_pressure + sex + smoking + anaemia, DEATH_EVENT ~ creatinine_phosphokinase + ejection_fraction + serum_creatinine  + serum_sodium)
# Ok, some type of fluke? It performed very well
buildEnsemble(mean, hf_records_dt, 10000, 0.8, DEATH_EVENT ~ age + high_blood_pressure + sex + smoking + anaemia, DEATH_EVENT ~ time + ejection_fraction + serum_creatinine  + serum_sodium)
buildEnsemble(mean, hf_records_dt, 10000, 0.8, DEATH_EVENT ~ age + high_blood_pressure + sex + smoking + anaemia + time, DEATH_EVENT ~  ejection_fraction + serum_creatinine  + serum_sodium)
# Let's try one more
getAvgLogisticAccuracy(hf_records_dt,  DEATH_EVENT ~ time + age + creatinine_phosphokinase  + ejection_fraction + high_blood_pressure + serum_creatinine + serum_sodium + smoking, iters=10000)
getAvgDecTreeAccuracy(hf_records_dt,  DEATH_EVENT ~ time + age + creatinine_phosphokinase  + ejection_fraction + high_blood_pressure + serum_creatinine + serum_sodium + smoking, iters=10000)

getAvgLogisticAccuracy(hf_records_dt,  DEATH_EVENT ~ time + age + creatinine_phosphokinase  + ejection_fraction + high_blood_pressure + serum_creatinine + serum_sodium, iters=10000)
getAvgDecTreeAccuracy(hf_records_dt,  DEATH_EVENT ~ time + age + creatinine_phosphokinase  + ejection_fraction + high_blood_pressure + serum_creatinine + serum_sodium, iters=10000)

getAvgLogisticAccuracy(hf_records_dt,  DEATH_EVENT ~ time + age + creatinine_phosphokinase  + ejection_fraction + serum_creatinine + serum_sodium, iters=10000)
getAvgDecTreeAccuracy(hf_records_dt,  DEATH_EVENT ~ time + age + creatinine_phosphokinase  + ejection_fraction + serum_creatinine + serum_sodium, iters=10000)

getAvgLogisticAccuracy(hf_records_dt,  DEATH_EVENT ~ time + age + ejection_fraction + serum_creatinine + serum_sodium, iters=10000)
getAvgDecTreeAccuracy(hf_records_dt,  DEATH_EVENT ~ time + age + ejection_fraction + serum_creatinine + serum_sodium, iters=10000)

getAvgLogisticAccuracy(hf_records_dt,  DEATH_EVENT ~ time + creatinine_phosphokinase  + ejection_fraction + serum_creatinine + serum_sodium, iters=10000)
getAvgDecTreeAccuracy(hf_records_dt,  DEATH_EVENT ~ time + creatinine_phosphokinase  + ejection_fraction + serum_creatinine + serum_sodium, iters=10000)

# Best performing formula for the given data
getAvgLogisticAccuracy(hf_records_dt,  DEATH_EVENT ~ time + ejection_fraction + serum_creatinine + serum_sodium, iters=10000)
getAvgDecTreeAccuracy(hf_records_dt,  DEATH_EVENT ~ time + ejection_fraction + serum_creatinine + serum_sodium, iters=10000)

getAvgLogisticAccuracy(hf_records_dt,  DEATH_EVENT ~ time + ejection_fraction + serum_sodium, iters=10000)
getAvgDecTreeAccuracy(hf_records_dt,  DEATH_EVENT ~ time + ejection_fraction + serum_sodium, iters=10000)

getAvgLogisticAccuracy(hf_records_dt,  DEATH_EVENT ~ time + ejection_fraction + serum_creatinine, iters=10000)
getAvgDecTreeAccuracy(hf_records_dt,  DEATH_EVENT ~ time + ejection_fraction + serum_creatinine, iters=10000)

# Gotta get SVM results now
getAvgSVMAccuracy(hf_records_raw_dt, DEATH_EVENT ~ time + ejection_fraction + serum_creatinine + serum_sodium, kernel="polynomial", iters=10000)

# For best models, craete one last test where we split and get the precision and recall measurements
final_index = sample(1:nrow(hf_records_dt), size=round(nrow(hf_records_dt)*0.80))
final_model = glm(DEATH_EVENT ~ time + ejection_fraction + serum_creatinine + serum_sodium, hf_records_dt[final_index,], family=binomial(link="logit"))
learning_table = confusionMatrix(table(round(final_model$fitted.values), final_model$y))

# Do performance measures on the
validation_preds = round(predict(final_model, hf_records_dt[!final_index,], type="response"))
validation_table = confusionMatrix(table(validation_preds, as.integer(unlist(hf_records_dt[ ! final_index, "DEATH_EVENT" ]))))
tmp_table = validation_table$table
